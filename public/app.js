...
function seasonLabel(s){const[y,m]=s.split('-').map(Number);return new Date(y,m-1,1).toLocaleDateString([],{month:'long',year:'numeric'})}
function daysLeftInSeason(){const d=new Date();return new Date(d.getFullYear(),d.getMonth()+1,0).getDate()-d.getDate()}
function readBattles(){const d=readState('battles',{wins:{},plays:{},battles:0,streak:0,best:0,lastWinner:null,lastChamp:null,season:currentSeason(),hall:[]});return {wins:{...d.wins||{}},plays:{...d.plays||{}},battles:d.battles|0,streak:d.streak|0,best:d.best|0,lastWinner:typeof d.lastWinner==='string'?d.lastWinner:null,lastChamp:typeof d.lastChamp==='string'?d.lastChamp:null,season:typeof d.season==='string'?d.season:currentSeason(),hall:Array.isArray(d.hall)?d.hall:[]}}
function rollSeason(){const cur=currentSeason();if(battleStats.season===cur)return;const prev=champId();
  if(prev&&battleStats.battles>0){const pm=allMemes().find(m=>m.id===prev);battleStats.hall=[{season:battleStats.season,champ:prev,wins:battleWins(prev)},...battleStats.hall].slice(0,12);setTimeout(()=>toast(prev?`📜 New season! “${pm?.title||'Cat'}” enters the hall of fame.`:'📜 New season! Fresh board.'),600)}
  battleStats={wins:{},plays:{},battles:0,streak:0,best:0,lastWinner:null,lastChamp:null,season:cur,hall:battleStats.hall};setLocal('battles',battleStats)}
...
const battleLosses=id=>Math.max(0,battlePlays(id)-battleWins(id));
const battleRate=id=>{const p=battlePlays(id);return p?Math.round(battleWins(id)/p*100):null};
function pickBattle(){const pool=allMemes();if(pool.length<2)return;let a=battleLeft,b=Math.random()<0.5?battleLeft:battleRight;
  for(let i=0;i<20;i++){const x=pool[Math.floor(Math.random()*pool.length)],y=pool[Math.floor(Math.random()*pool.length)];if(x.id!==y.id&&(x.id!==a?.id||y.id!==b?.id)){a=x;b=y;break}}
  if(!a||!b||a.id===b.id){const s=shuffle(pool);a=s[0];b=s[1]}
  battleLeft=a;battleRight=b;
}
const battleTaunts=['The crowd goes mild.','A historic moment for cats.','Democracy works. No recount needed.','The chimp nods approvingly.','Questionable behavior, rewarded.','A landslide. Somehow still about naps.','The other cat demands a recount. Denied.','Instant classic. Frame it.','The judges (you) have spoken.','That one had main-character energy.','Science can’t explain this one.','Straight into the hall of fame.'];
...
let labTop=null,labBottom=null,labCat=null,labEngine=null;
const randMeme=()=>allMemes()[Math.floor(Math.random()*allMemes().length)];
function rollLab(part){if(!allMemes().length)return;labEngine=null;const withTop=()=>{let m=randMeme();for(let i=0;i<10&&!m.top;i++)m=randMeme();return m};const withBottom=()=>{let m=randMeme();for(let i=0;i<10&&!m.bottom;i++)m=randMeme();return m};
  if(!labTop||part==='top'||part==='all')labTop=withTop();if(!labBottom||part==='bottom'||part==='all')labBottom=withBottom();if(!labCat||part==='cat'||part==='all')labCat=randMeme();renderLab()}
function labMeme(){return {title:`${labTop?.title||'Top'} × ${labBottom?.title||'Bottom'}`,top:labEngine?.top||labTop?.top||'',bottom:labEngine?.bottom||labBottom?.bottom||'',image:labCat?.image||'',template:labCat?`Frankenmeme (${labCat.template})`:'Frankenmeme',mood:labCat?.mood||'chaos'}}
function curseRating(){const n=(labTop?.top.length||0)+(labBottom?.bottom.length||0)*3+(labCat?.id.length||0)*7;return (n%5)+1}
function renderLab(){if(!labTop||!labBottom||!labCat){rollLab();return}const m=labMeme();$('#lab-preview').innerHTML=visual(m,'eager');
  const stars='★'.repeat(curseRating())+'☆'.repeat(5-curseRating());
  const names=['blessed','mildly cursed','cursed','deeply cursed','FORBIDDEN'];
  $('#lab-provenance').innerHTML=`<span>Above the cat: <strong>${labEngine?'🤖 The Engine':esc(labTop.title)}</strong></span><span>Below the cat: <strong>${labEngine?'🤖 The Engine':esc(labBottom.title)}</strong></span><span>The cat: <strong>${esc(labCat.title)}</strong></span><span class="curse">${stars} ${names[curseRating()-1]}</span>`}
function inspireLab(){if(!labTop||!labBottom||!labCat)rollLab();labEngine=inspireCaption();renderLab();sfx('blip');toast('The machine dreamed this. No refunds.')}
...
let pack=readPack(),packTab='haul',packStage='sealed',packReveal=null;
...
function packList(){let list=packTab==='loved'?allMemes().filter(m=>saved.has(m.id)):allMemes().filter(m=>pack.haul[m.id]);
  ...
  if(query){const terms=query.toLowerCase().trim().split(/\s+/);list=list.filter(m=>{const hay=[m.title,m.top,m.bottom,m.template,m.mood,...m.tags].join(' ').toLowerCase();return terms.every(t=>hay.includes(t))})}
  return list}
function renderPacks(){const panel=$('#pack-panel');if(!panel)return;
  ...
  $('#pack-stats').innerHTML=`<div class="stat-chip" title="Consecutive days with a pack opened"><strong><span data-count="${pack.streak}">0</span> 🔥</strong>day streak</div><div class="stat-chip" title="Your longest opening run"><strong><span data-count="${pack.best}">0</span> 🏆</strong>best streak</div><div class="stat-chip" title="Total cats pulled"><strong><span data-count="${haulSize()}">0</span></strong>cats hauled</div>`;countUp($('#pack-stats'));
  const loved=allMemes().filter(m=>saved.has(m.id)).length;
  $('#pack-tabs').innerHTML=`<button class="${packTab==='haul'?'active':''}" data-packtab="haul">📦 Haul · ${haulSize()}</button><button class="${packTab==='loved'?'active':''}" data-packtab="loved">♥ Loved · ${loved}</button>`;
  renderForge();
  const list=packList().slice(0,60);
  $('#haul-grid').innerHTML=list.length?list.map((m,i)=>packCard(m,i)).join(''):`<p class="leader-empty">${packTab==='loved'?'No loved cats yet — tap ♥ on any meme.':'No pulls yet. The pack awaits above.'}</p>`;
...
async function sharePull(id){const m=allMemes().find(m=>m.id===id);if(!m)return;
  await document.fonts.ready;let img;try{img=await loadImage(m.image)}catch{toast('That cat refuses to travel.');return}
  ...
  let ctx=canvas.getContext('2d');ctx.font=`800 ${fontSize}px "DM Sans", sans-serif`;
  ...
  ctx.fillStyle='#F7F5EE';ctx.font='800 34px "DM Sans", sans-serif';ctx.fillText('CATMEMECHIMP · DAILY DROP',pad,40);
  ...
  ctx.fillStyle='#fff';ctx.fillRect(0,headH,width,th+imageH+bh);
  ctx.fillStyle='#24231e';ctx.font=`800 ${fontSize}px "DM Sans", sans-serif`;
  ...
  ctx.fillStyle='#B9C5F3';ctx.font='500 22px "DM Sans", sans-serif';ctx.fillText('ripped from a daily pack · good cats, questionable behavior',pad,headH+th+imageH+bh+66);
  const blob=await new Promise(res=>canvas.toBlob(res,'image/png'));if(!blob){toast('The card couldn’t be made.');return}
  const file=new File([blob],'catmeme-drop.png',{type:'image/png'});
  try{if(navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({files:[file],title:'My daily cat'});toast('Shared! Go forth and convert friends.');return}}catch(e){if(e&&e.name==='AbortError')return}
  const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`catmeme-drop-${m.id}.png`;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),5000);toast('Card downloaded. Spread the gospel.')}
...
function renderArena(){if(!battleLeft||!battleRight)pickBattle();if(!battleLeft||!battleRight)return;
  $('#arena-stats').innerHTML=`<div class="stat-chip" title="Total matchups you have judged"><strong><span data-count="${battleStats.battles}">0</span></strong>battles judged</div><div class="stat-chip" title="Consecutive crowns without skipping"><strong><span data-count="${battleStats.streak}">0</span> 🔥</strong>current streak</div><div class="stat-chip" title="Your longest judging run"><strong><span data-count="${battleStats.best}">0</span> 🏆</strong>best streak</div><div class="stat-chip" title="Boards reset every month; past champions live below"><strong>${seasonLabel(battleStats.season).split(' ')[0]}</strong>${daysLeftInSeason()}d left</div>`;countUp($('#arena-stats'));
  const bc=$('#battle-count');if(bc)bc.textContent=battleStats.battles;
...
function updateCounts(){$('#all-count').textContent=library.length;const dc=$('#daily-count');if(dc)dc.textContent=canOpenPack()?'🎁':haulSize();$('#created-count').textContent=created.length;const bc=$('#battle-count');if(bc)bc.textContent=battleStats.battles}
function render(){updateCounts();$$('[data-page]').forEach(b=>{b.classList.toggle('active',b.dataset.page===page);if(b.dataset.page===page)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current')});
  ...
  $('#breadcrumb').textContent=({discover:'THE CAT ARCHIVE',arena:'THE BATTLE ARENA',lab:'THE FRANKENMEME LAB',packs:'THE DAILY DROP',saved:'SAVED FOR A RAINY DAY',collections:'HANDPICKED CAT COLLECTIONS',studio:'THE NONSENSE LAB'})[page]||'THE CAT ARCHIVE';
  $('#section-eyebrow').textContent=({discover:'THE GOOD STUFF',arena:'TWO CATS ENTER. ONE CAT LEAVES.',lab:'MAD SCIENCE, BUT CATS.',packs:'A LITTLE GAMBLE.',saved:'YOUR PERSONAL SEROTONIN STASH',collections:'A CAT FOR EVERY OCCASION',studio:'MADE BY YOU. APPROVED BY YOU.'})[page];
  $('#library-title').textContent=page==='arena'?'Crown the funniest cat.':page==='lab'?'Build an abomination.':page==='packs'?"Today's drop.":query?`Cats matching “${query}”`:({discover:'Find your feline frequency.',saved:'The keepers.',collections:'Good things come in collections.',studio:'Your little masterpieces.'})[page];
  const desc=({discover:'',arena:'250 memes. Zero mercy. Pick a winner, build a streak — skips reset the streak.',lab:'Three cats walk in. One meme walks out. Reroll parts until it feels illegal.',packs:'One sealed pack per human per 24 hours. Rip it, keep the cat, chase the streak.',saved:'Every cat you’ve hearted, right here when you need one.',collections:'Small, handpicked corners of the archive. Pick one and settle in.',studio:'Your captions. Your cats. Your very questionable sense of humor.'})[page];$('#page-description').textContent=desc;$('#page-description').hidden=!desc;
  const inArena=page==='arena',inLab=page==='lab',inPacks=page==='packs',hideGrid=inArena||inLab||inPacks;
  $('#arena').hidden=!inArena;$('#lab').hidden=!inLab;$('#packs').hidden=!inPacks;$('#shuffle').hidden=hideGrid;
...
function buddyInit(){const w=$('#chimp-buddy');if(!w)return;w.hidden=false;w.classList.toggle('muted',buddyMuted);
  ...
  for(const ev of ['pointerdown','keydown'])document.addEventListener(ev,poke,{passive:true,capture:true});poke();buddy3D()}
function buddy3D(){const btn=$('#buddy-btn');if(!btn||!matchMedia('(pointer:fine)').matches||matchMedia('(prefers-reduced-motion: reduce)').matches)return;let raf=null;addEventListener('pointermove',e=>{if(raf)return;raf=requestAnimationFrame(()=>{raf=null;const r=btn.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,dx=Math.max(-1,Math.min(1,(e.clientX-cx)/220)),dy=Math.max(-1,Math.min(1,(e.clientY-cy)/220));btn.style.transform=`rotateY(${(dx*22).toFixed(1)}deg) rotateX(${(-dy*22).toFixed(1)}deg)`;btn.style.setProperty('--gx',`${50+dx*40}%`);btn.style.setProperty('--gy',`${50+dy*40}%`)})},{passive:true})}
...
function shuffledGrid(){order=shuffle(allMemes()).map(m=>m.id);$('#sort').value='curated';limit=18;sfx('chaos');render();toast('Same good cats. A little new chaos.')}
...
function enterTV(){if(!viewerQueue.length)return;tvReturn={queue:[...viewerQueue],index:viewerIndex};tvMode=true;tvChannel='all';tvPlays=0;showViewer();restartAutoplay();zap();ident(`📺 CAT TV`,`${library.length} CATS · ALL DAY · ALL NIGHT`)}
function exitTV(){tvMode=false;tvSleep=0;$('#viewer').classList.remove('tv');$('#tv-overlay').hidden=true;$('#screen').hidden=true;stopAutoplay();if(tvReturn){viewerQueue=tvReturn.queue;viewerIndex=Math.min(tvReturn.index,Math.max(0,viewerQueue.length-1));tvReturn=null;if($('#viewer').open)showViewer()}}
const TV_SPEEDS=[3,6,10],TV_SLEEPS=[0,10,20,30];
let tvChannel='all',tvSpeedIdx=1,tvSleepIdx=0,tvSleep=0,tvReturn=null,tvPlays=0,identT=null;
...
const ADS=[
 {brand:'Potassium™',line:'For cats with places to be.',img:'/assets/memes/7s8bhl.jpg',cta:'ASK YOUR HUMAN'},
...
function tvTick(){if(tvSleep>0){tvSleep--;const m=currentMeme();if(m)updateTVOverlay(m);if(tvSleep<=0){signOff();return}}tvPlays++;if(tvPlays%5===0){showAd()}else{browse(1)}}
function cycleVol(){volIdx=(volIdx+1)%VOLS.length;masterVol=VOLS[volIdx];setLocal('vol',masterVol);updateTVOverlay(currentMeme());sfx('blip')}
...
function setTVChannel(d){const i=(moods.findIndex(m=>m[0]===tvChannel)+d+moods.length)%moods.length;tvChannel=moods[i][0];const pool=tvChannel==='all'?[...allMemes()]:allMemes().filter(m=>m.mood===tvChannel);if(!pool.length)return;viewerQueue=shuffle(pool);viewerIndex=0;showViewer();zap();sfx('blip');ident(`CH${i+1} · ${moods[i][1]} ${moods[i][2]}`,tvChannel==='all'?`${library.length} CATS · NO MERCY`:`${pool.length} CATS · ALL ${moods[i][2].toUpperCase()}`)}
...
function shortcuts(){info('Less clicking. More cats.',`<table class="shortcut-table"><tbody><tr><td>/</td><td>Jump to search</td></tr><tr><td>R</td><td>A random cat from this view</td></tr><tr><td>← / →</td><td>Previous / next cat in the viewer, or vote in the Arena</td></tr><tr><td>T / B / C / F</td><td>Reroll Frankenmeme top / bottom / cat / full chaos</td></tr><tr><td>↑ / ↓</td><td>Change TV channel in Cat TV mode</td></tr><tr><td>O</td><td>Open today's pack on the Daily Drop page</td></tr><tr><td>M</td><td>Mute / unmute sounds</td></tr><tr><td>S</td><td>Save the cat in the viewer</td></tr><tr><td>Space</td><td>Pause / resume autoplay in the viewer</td></tr><tr><td>Esc</td><td>Close the current panel</td></tr><tr><td>?</td><td>This little cheat sheet</td></tr></tbody></table><p>On your phone, swipe left or right on a meme in the viewer. Tap a cat to crown it in the Arena. Autoplay changes cats every six seconds and pauses when you leave the tab.</p>`)}
document.addEventListener('click',e=>{const nav=e.target.closest('[data-page]');if(nav){navigate(nav.dataset.page);return}const vt=e.target.closest('[data-vote]');if(vt){voteBattle(vt.dataset.vote);return}const open=e.target.closest('[data-open]');if(open){openViewer(open.dataset.open,page==='arena'?allMemes():filterMemes());return}const save=e.target.closest('[data-save]');if(save){toggleSave(save.dataset.save);return}const mb=e.target.closest('[data-mood]');if(mb?.classList.contains('mood-filter')){mood=mb.dataset.mood;limit=18;render();return}const cb=e.target.closest('[data-collection]');if(cb){collection=collection===cb.dataset.collection?'':cb.dataset.collection;mood='all';limit=18;render();return}const ds=e.target.closest('[data-delset]');if(ds){const s=setById(ds.dataset.delset);deleteSet(ds.dataset.delset);render();toast(s?`“${s.name}” released into the wild.`:'Set deleted.');return}const cr=e.target.closest('[data-collect]');if(cr){const added=toggleInSet(cr.dataset.collect,currentMeme()?.id);renderCollect();updateCounts();toast(added?'Stashed. A fine addition.':'Removed from that set.');return}const rm=e.target.closest('[data-remove-from-set]');if(rm){toggleInSet(collection,rm.dataset.removeFromSet);render();return}const po=e.target.closest('#pack-open');if(po){openPack();return}const pt=e.target.closest('[data-packtab]');if(pt){packTab=pt.dataset.packtab;renderPacks();return}const sh=e.target.closest('[data-share]');if(sh){sharePull(sh.dataset.share);return}const mz=e.target.closest('[data-melt]');if(mz){meltDupe(mz.dataset.melt);return}if(e.target.closest('[data-forge]')){forgePack();return}});
...
function toggleTheme(){const next=document.documentElement.dataset.theme==='dark'?'light':'dark';setLocal('theme',next);applyTheme(next);toast(next==='dark'?'Lights out. The cats can see in the dark.':'Lights on. The cats judge you clearly again.')}
...
$('#viewer-image-wrap').addEventListener('click',()=>{if(tvMode)browse(1)});
