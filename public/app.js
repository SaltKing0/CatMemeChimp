...
function renderPacks(){const panel=$('#pack-panel');if(!panel)return;
  ...
  $('#pack-stats').innerHTML=`<div class="stat-chip" title="Consecutive days with a pack opened"><strong>${pack.streak} 🔥</strong>day streak</div><div class="stat-chip" title="Your longest opening run"><strong>${pack.best} 🏆</strong>best streak</div><div class="stat-chip" title="Total cats pulled"><strong>${haulSize()}</strong>cats hauled</div>`;
...
function renderArena(){if(!battleLeft||!battleRight)pickBattle();if(!battleLeft||!battleRight)return;
  $('#arena-stats').innerHTML=`<div class="stat-chip" title="Total matchups you have judged"><strong>${battleStats.battles}</strong>battles judged</div><div class="stat-chip" title="Consecutive crowns without skipping"><strong>${battleStats.streak} 🔥</strong>current streak</div><div class="stat-chip" title="Your longest judging run"><strong>${battleStats.best} 🏆</strong>best streak</div><div class="stat-chip" title="Boards reset every month; past champions live below"><strong>${seasonLabel(battleStats.season).split(' ')[0]}</strong>${daysLeftInSeason()}d left</div>`;
