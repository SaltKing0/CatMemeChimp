...
async function importSets(file){if(!file)return;try{const data=JSON.parse(await file.text());const arr=Array.isArray(data)?data:data.sets;if(!Array.isArray(arr))throw new Error('bad file');const known=new Set(allMemes().map(m=>m.id));let added=0;
  ...
  if(!added)throw new Error('bad file');saveSets();if(page==='collections')render();toast(`Imported ${added} ${added===1?'set':'sets'}. Sharing is caring.`)}catch{toast('That file isn’t a CatMemeChimp set export.')}}
...
document.addEventListener('change',e=>{if(e.target.id==='import-sets'){importSets(e.target.files[0]);e.target.value=''}});
...
$('#upload').onchange=()=>uploadCat($('#upload').files[0]);$('#editor-download').onclick=()=>download(editorMeme(),$('#editor-download'));$('#editor-motion').onclick=()=>exportMotion(editorMeme(),$('#editor-motion'));$('#lab-motion').onclick=()=>exportMotion(labMeme(),$('#lab-motion'));
