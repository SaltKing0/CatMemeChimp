...
const hotScore=id=>battlePlays(id)+2*battleWins(id);
function hotIds(){return new Set([...allMemes()].filter(m=>battlePlays(m.id)>=3).sort((a,b)=>hotScore(b.id)-hotScore(a.id)).slice(0,12).map(m=>m.id))}
...
function render(){updateCounts();$$('[data-page]').forEach(b=>{b.classList.toggle('active',b.dataset.page===page);if(b.dataset.page===page)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current')});
  ...
  $('#library-title').textContent=page==='arena'?'Crown the funniest cat.':page==='lab'?'Build an abomination.':page==='packs'?"Today's drop.":templateFilter?`More ${tmplName}.`:query?`Cats matching “${query}”`:({discover:'Find your feline frequency.',saved:'The keepers.',collections:'Good things come in collections.',studio:'Your little masterpieces.'})[page];
 ...
 $('#mood-filters').innerHTML=moods.map(([id,symbol,label])=>`<button class="mood-filter ${mood===id?'active':''}" data-mood="${id}" aria-pressed="${mood===id}"><span aria-hidden="true">${symbol}</span>${label}</button>`).join('');
  ...
  $('#result-count').innerHTML=list.length?`<strong>${list.length} ${list.length===1?'cat':'cats'}</strong> ${page==='saved'?'worth keeping':mood==='all'&&!query&&!collection?'and not a single thought':collection?'in this collection':'in this corner'} <span class="shown-count">· showing ${display.length}</span>${templateFilter?' · <button class="leader-open" data-tclear>all templates</button>':''}`:'0 cats found';
...
function navigate(p){page=p;mood='all';query='';collection='';templateFilter=null;limit=18;$('#search').value='';if(p==='arena'&&(!battleLeft||!battleRight))pickBattle();if(p==='lab'&&(!labTop||!labBottom||!labCat))rollLab();if(p==='packs'&&canOpenPack())setTimeout(()=>buddySay('packReady'),800);render();window.scrollTo({top:0,behavior:'smooth'})}
