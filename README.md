...
The 60 captions were written for CATMEMECHIMP. Images are community meme templates sourced from Imgflip; every record in **public/library.json** retains its source page. No license or ownership of third-party source imagery is asserted. Collection date: September 8, 2026. This is a curated collection, not a live feed. See [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) before redistributing media.
...
Keyboard: `/` search, `R` random cat, `?` shortcuts, `←`/`→` browse in viewer, `S` save in viewer, `Space` toggle autoplay, `Esc` close. Autoplay pauses when the tab is hidden.
...
The MIT license applies to the original source code and CATMEMECHIMP branding in this repository. It does not relicense third-party meme templates or fonts; see the third-party notices.
